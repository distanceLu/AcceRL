...
class FinetuneConfig:
    ...
    # Logging
    tensorboard_logs: Path = Path("logs")  # Directory to store TensorBoard logs
...
def run_forward_pass(
    ...
    # Compute metrics for continuous action representations (L1 regression | diffusion)
        ...
        predicted_curr_action = action_all.detach()[:, 0]
        ground_truth_next_actions = ground_truth_actions[:, 1:]
        predicted_next_actions = action_all.detach()[:, 1:]
        ...
        metrics.update(
            {
                "nll_loss_value": nll_loss.item(),  # Detached value for logging
                "l1_sample_loss_value": l1_loss_sample.item(),
                "l1_mu_loss_value": l1_loss_mu.item(),
                "curr_action_l1_loss": curr_action_l1_loss.item(),
                "next_actions_l1_loss": next_actions_l1_loss.item(),
...
def count_trainable_params(module, name):
    """一个辅助函数，用于计算并打印模块中可训练参数的数量。"""
    if module is None:
        print(f"Module '{name}' is None. Skipping.")
        return
    params = sum(p.numel() for p in module.parameters() if p.requires_grad)
    print(f"Trainable params in '{name}': {params:,}")
...
def finetune(cfg: FinetuneConfig) -> None:
    ...
    # Initialize wandb logging and tensorboard logging
    if distributed_state.is_main_process:
        # wandb.init(entity=cfg.wandb_entity, project=cfg.wandb_project, name=f"ft+{run_id}")

        tensorboard_log_dir = cfg.tensorboard_logs / cfg.run_root_dir.name
    ...
    if cfg.use_lora:
        actor.setup_finetuning(lora_rank=cfg.lora_rank, lora_dropout=cfg.lora_dropout)

    actor = actor.to(device_id)
    actor = wrap_ddp(actor, device_id, find_unused=True)
    # check_unnorm_key(cfg, actor.vla)

    print("\nCollecting parameters for the optimizer...")
    parameter_groups = actor.module.get_parameter_groups()
    
    trainable_params = []
    for group in parameter_groups:
        # 只选择 'policy' 和 'lora' 组
        if group and group['name'] in ['policy', 'lora']:
            trainable_params.extend(group['params'])
            print(f"# Found and added '{group['name']}' group with {sum(p.numel() for p in group['params'])} params.")
        else:
            print(f"# Skipping '{group['name']}' group.")

    if not trainable_params:
        raise ValueError("No trainable parameters were found! Check your model setup.")
    
    print(f"\n# Total trainable params for optimizer: {sum(p.numel() for p in trainable_params)}")
    optimizer = AdamW(trainable_params, lr=cfg.learning_rate)
    
    # 1. Action Head
    count_trainable_params(actor.module.action_head, "Action Head")
    ...
    ## 注意这里的 use lora和 use film 代码没有删去，因为在 actor-critic 类中 use film是默认关闭的，没有加载 lora 和 film
    # LoRA setup
    # if cfg.use_lora:
    ...
    # if cfg.use_proprio:
    #     proprio_projector = actor.proprio_projector.to(device_id)

    # # If applicable, instantiate continuous action head for L1 regression
    # if cfg.use_l1_regression:
    #     action_head = actor.action_head.to(device_id)
    ...
    # NUM_PATCHES = actor.vla.module.vision_backbone.get_num_patches() * actor.vla.module.vision_backbone.get_num_images_in_input()
    ...
    # lora_params = [p for p in actor.vla.parameters() if p.requires_grad]
    # if lora_params:
    #     trainable_params.extend(lora_params)
    #     print(f"# Found and added LoRA with {sum(p.numel() for p in lora_params)} params.")
    ...
    train_dataset = RLDSDataset(
        ...
        cfg.dataset_name,
        batch_transform,
        resize_resolution=tuple(actor.module.vla.config.image_sizes),
    ...
    recent_metrics = {
        ...
        "nll_loss_value": deque(maxlen=cfg.grad_accumulation_steps),
        "l1_sample_loss_value": deque(maxlen=cfg.grad_accumulation_steps),
        "l1_mu_loss_value": deque(maxlen=cfg.grad_accumulation_steps),
        "entropy": deque(maxlen=cfg.grad_accumulation_steps),
    }
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            if gradient_step_idx > 0 and log_step % cfg.save_freq == 0:
                save_training_checkpoint(
                    ...
                    run_dir=run_dir,
                    log_step=log_step,
                    vla=actor.module.vla,
                    processor=processor,
                    proprio_projector=proprio_projector if cfg.use_proprio else None,
                    # noisy_action_projector=noisy_action_projector if cfg.use_diffusion else None,
                    action_head=action_head if (cfg.use_l1_regression or cfg.use_diffusion) else None,
                    log_std_head=log_std_head if (cfg.use_l1_regression or cfg.use_diffusion) else None,
