...
from storm.attention_blocks import get_vector_mask
from storm.attention_blocks import PositionalEncoding1D, AttentionBlock, AttentionBlockKVCache
...
class StochasticTransformerKVCache(nn.Module):
    def __init__(self, stoch_dim, action_dim, instruction_dim, feat_dim, num_layers, num_heads, max_length, dropout, dist):
        ...
        # mix image_embedding and action
        self.stem = nn.Sequential(
            nn.Linear(stoch_dim+action_dim+instruction_dim, feat_dim, bias=False),
            nn.LayerNorm(feat_dim),
            nn.ReLU(inplace=True),
            nn.Linear(feat_dim, feat_dim, bias=False),
            nn.LayerNorm(feat_dim)
