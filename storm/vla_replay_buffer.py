...
class ReplayBuffer():
    def __init__(self, obs_shape, num_envs, action_dim, dist, max_length=int(1E6), warmup_length=50000, store_on_gpu=False) -> None:
        ...
        if store_on_gpu:
            ...
            self.obs_buffer = [[] for _ in range(num_envs)] * (max_length//num_envs)
            if dist == "onehot":
    ...
    def sample(self, batch_size, external_batch_size, batch_length, to_device="cuda"):
        if self.store_on_gpu:
            ...
            if batch_size > 0:
                for i in range(self.num_envs):
                    indexes = np.random.randint(0, self.length+1-batch_length, size=batch_size//self.num_envs)
                    # obs.append([self.obs_buffer[idx:idx+batch_length, i] for idx in indexes])
                    for idx in indexes:
                        for j in range(batch_length):
                            obs.append(self.obs_buffer[idx+j][i])  # flat方便预处理
