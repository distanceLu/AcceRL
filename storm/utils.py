...
class Logger():
    ...
    def log(self, tag, value, current_steps):
        if tag not in self.tag_step:
            self.tag_step[tag] = 0
        else:
            self.tag_step[tag] += 1
        if "video" in tag:
            self.writer.add_video(tag, value, current_steps, fps=15)
        elif "images" in tag:
            self.writer.add_images(tag, value, current_steps)
        elif "hist" in tag:
            self.writer.add_histogram(tag, value, current_steps)
        else:
            self.writer.add_scalar(tag, value, current_steps)
