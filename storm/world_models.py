...
class WorldModel(nn.Module):
    def __init__(self, in_channels, action_dim, instruction_dim,
                 transformer_max_length, transformer_hidden_dim, transformer_num_layers, transformer_num_heads, dist):
        super().__init__()
        self.dist = dist
    ...
    def calc_last_dist_feat(self, latent, action, instruction):
        with torch.autocast(device_type='cuda', dtype=torch.bfloat16, enabled=self.use_amp):
            temporal_mask = get_subsequent_mask(latent)
            dist_feat = self.storm_transformer(latent, action, temporal_mask, instruction)
    ...
    def predict_next(self, last_flattened_sample, action, instruction, log_video=True):
        with torch.autocast(device_type='cuda', dtype=torch.bfloat16, enabled=self.use_amp):
            dist_feat = self.storm_transformer.forward_with_kv_cache(last_flattened_sample, action, instruction)
    ...
    def init_imagine_buffer(self, imagine_batch_size, imagine_batch_length, dtype):
        ...
        if self.imagine_batch_size != imagine_batch_size or self.imagine_batch_length != imagine_batch_length:
            ...
            self.instruction_buffer = torch.zeros(instruction_size, dtype=dtype, device="cuda")

    def imagine_data(self, agent: agents.ActorCriticAgent, sample_obs, sample_action, sample_instruction,
        ...
        for i in range(imagine_batch_length):
            ...
            last_obs_hat, last_reward_hat, last_termination_hat, last_latent, last_dist_feat = self.predict_next(
                self.latent_buffer[:, i:i+1], self.action_buffer[:, i:i+1], self.instruction_buffer, log_video=log_video)
    ...
    def update(self, obs, action, reward, termination, instruction, current_steps, logger=None):
        self.train()
