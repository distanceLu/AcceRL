...
ensure_session_runtime_dirs() {
  ...
  if [[ ! -f "${session_dir}/launch_history.tsv" ]]; then
    printf 'timestamp\tjob_key\ttask_name\tclip_mode\tsigma\tseed\tgpu_id\tpid\tlog_file\n' > "${session_dir}/launch_history.tsv"
  fi
}
...
create_jobs_manifest() {
  ...
  printf 'job_index\tjob_key\ttask_name\tclip_mode\tsigma\tseed\texp_name\ttask_runs_root\ttrain_iters\n' > "${jobs_file}"

  local task_name clip_mode seed sigma sigma_tag job_key task_runs_root
      ...
      if [[ "${clip_mode}" == "gipo" ]]; then
        for sigma in "${gipo_sigmas_array[@]}"; do
          ...
          for seed in "${seed_array[@]}"; do
            ...
            job_key="${BASE_EXP_NAME}_${task_name}_seed${seed}_${clip_mode}_${sigma_tag}"
            printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
              "${JOB_TOTAL_COUNT}" "${job_key}" "${task_name}" "${clip_mode}" "${sigma}" "${seed}" \
...
parse_status_snapshot() {
  ...
  local job_index job_key task_name clip_mode sigma seed status pid pid_alive gpu_id last_iter target_iters progress_ratio done_reason run_dir stale_run_dirs log_file
  while IFS=$'\t' read -r job_index job_key task_name clip_mode sigma seed status pid pid_alive gpu_id last_iter target_iters progress_ratio done_reason run_dir stale_run_dirs log_file; do
...
append_launch_history() {
  ...
  local log_file="$9"
  printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
    "${timestamp}" "${job_key}" "${task_name}" "${clip_mode}" "${sigma}" "${seed}" "${gpu_id}" "${pid}" "${log_file}" \
...
launch_job() {
  ...
  log "[launch] task=${task_name} clip_mode=${clip_mode} sigma=${sigma} seed=${seed} gpu=${gpu_id}"
  nohup "${cmd[@]}" > "${log_file}" 2>&1 &

  local pid=$!
  printf '%s\n' "${pid}" > "${pid_file}"
  printf '%s\n' "${pid}" >> "${CURRENT_SESSION_DIR}/all_pids.txt"
  append_launch_history "${launched_at}" "${job_key}" "${task_name}" "${clip_mode}" "${sigma}" "${seed}" "${gpu_id}" "${pid}" "${log_file}"
  write_meta_file "${meta_file}" "${job_key}" "${task_name}" "${clip_mode}" "${sigma}" "${seed}" "${gpu_id}" "${pid}" "${log_file}" "${launched_at}"
...
launch_missing_jobs() {
  ...
  local status_file="${CURRENT_SESSION_DIR}/status_snapshot.tsv"
  local job_index job_key task_name clip_mode sigma seed status pid pid_alive current_gpu_id last_iter target_iters progress_ratio done_reason run_dir stale_run_dirs log_file
  while IFS=$'\t' read -r job_index job_key task_name clip_mode sigma seed status pid pid_alive current_gpu_id last_iter target_iters progress_ratio done_reason run_dir stale_run_dirs log_file; do
    [[ "${job_index}" == "job_index" ]] && continue
    ...
    launch_job "${job_key}" "${task_name}" "${clip_mode}" "${sigma}" "${seed}" "${selected_gpu}"
