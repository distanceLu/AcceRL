...
* **像素级高保真推演**：系统不依赖于压缩的潜在空间，而是利用 `DenoiserInferenceActor` (基于 Diffusion 的转移模型) 预测下一帧高保真像素级观测，并交由 `RewardInferenceActor` 输出成功率分布与终止信号，从而在 GPU 上完成纯张量级别的虚拟环境步进。
* **极致样本效率**：该机制彻底避免了与缓慢物理引擎的长时交互，将 VLA 模型的在线样本效率提升了惊人的 **200倍 (20,000%)**。
