...
def log_local(batch_logs, save_dir, filename, save_fps=10, rescale=True):
    ...
    for key in batch_logs:
        ...
        if isinstance(value, list) and isinstance(value[0], str):
            ...
            video = video.permute(2, 0, 1, 3, 4) # t,n,c,h,w
            if video.shape[0] == 1:
                img = video[0].permute(0,2,3,1).reshape(-1, video.shape[4], video.shape[2])
                img = np.uint8(255. * (img.data.cpu().numpy() + 1) / 2)
                ...
                torchvision.io.write_video(path, grid, fps=save_fps,)
