...
这是从 `ds_libero_gipo_discrete.py` 中抽出的一个最小化、可独立运行的分布式 GIPO 框架示例。它保留了核心 Ray actor 拓扑、GIPO 数学逻辑，以及 DeepSpeed/ZeRO 训练路径，但把沉重的 OpenVLA / Libero 依赖替换成了：
...
当前版本默认开启 DeepSpeed，并使用 `deepspeed.initialize`（含 `ds_config` 与 ZeRO）：
...
## ds_com Standalone 版本

新增文件：`main_ray_gipo_ds_standalone.py`。该版本保留 `TrainerActorCom` / `InferenceActorCom` 通讯骨架和广播握手流程，仅替换为 fake env / fake model。

运行示例：

```bash
cd /cpfs01/qianfy_workspace/openvla_oft_rl/minimal_ray_ppo
python main_ray_gipo_ds_standalone.py --train-iters 5 --num-rollout-workers 1 --num-inference-actors 1
```

TensorBoard 日志目录默认是 `runs/minimal_ray_gipo`，可用以下命令查看：
...
- 其余重型依赖（如 OpenVLA、Libero）已替换为 fake 组件
- 目标是帮助你在离线本地机器上研究：
  - actor 拓扑
