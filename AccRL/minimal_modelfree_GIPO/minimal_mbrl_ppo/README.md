...
这是一个从 `rl/ds_wm_discrete_diffusion.py` 抽取出的最小化、可独立运行的“世界模型推演 + 分布式 GIPO”示例。它保留了核心 Ray actor 架构和 imagination rollout 逻辑，但移除了 OpenVLA、Libero、真实奖励模型、真实扩散采样器、DeepSpeed 以及图像预处理依赖。
...
- `TrainerActor`
  ...
  - 执行普通 PyTorch 优化，不再依赖 DeepSpeed
  - 保留 GIPO 的核心数学：
    - advantage 标准化
    - clipped objective
    - value loss
