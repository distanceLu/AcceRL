...
def configure_opt(model: nn.Module, lr: float, weight_decay: float, eps: float, *blacklist_module_names: str) -> AdamW:
    ...
    # 创建一个模块名到模块对象的映射，用于快速查找
    module_dict = dict(model.named_modules())
    
    # 直接遍历所有参数，确保参数名一致
    for pn, p in model.named_parameters():
        # 找到这个参数所属的模块
        # 参数名格式：module.submodule.param_name
        # 我们需要找到最直接的父模块
        parts = pn.rsplit('.', 1)
        if len(parts) == 2:
        ...
        # 使用 if-elif 链确保每个参数只被处理一次
        # 优先级：blacklist_module_names > bias > blacklist modules > whitelist modules > embedding > default
        
        if any([pn.startswith(module_name) for module_name in blacklist_module_names]):
            # 首先检查是否在 blacklist_module_names 中（最高优先级）
            no_decay.add(pn)
            ...
            # 注意：这个检查要在 whitelist 之前，因为 LayerNorm 可能在 TransformerEncoderLayer 内部
            no_decay.add(pn)
            ...
            # embedding-like parameters should not be decayed
            no_decay.add(pn)
    ...
    # For any remaining parameters, apply default rules:
    # - weights should be decayed
    # - biases and embeddings should not be decayed
    for pn in missing_params:
        if "bias" in pn or "embedding" in pn.lower():
            ...
            # Default: treat as weight and apply decay
            decay.add(pn)
...
def load_configs():
    agent_config_path = Path("/cpfs01/jinshiji_workspace/diamond/config/agent/default.yaml")
    trainer_config_path = Path("/cpfs01/jinshiji_workspace/diamond/config/trainer.yaml")
    
    agent_cfg = OmegaConf.load(agent_config_path)
    trainer_cfg = OmegaConf.load(trainer_config_path)
    
    return agent_cfg, trainer_cfg