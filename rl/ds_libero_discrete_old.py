...
class RolloutWorkerActor(BaseWorkerActor):
    def __init__(self, infer, replay, wid, stats_actor, cfg, benchmark_name, gamma, lambda_, reward_scale, torch_dtype, rollout_local_buf, task_ids):
...
class TrainerActor(TrainerActorCom):
    def __init__(self, rank, world_size, replay_buffer, cfg, train_batch_size, accumulation_steps, 
                 ...
                 policy_train_start_step, train_iters, clip_mode, sigma):
        super().__init__()
        self.rank = rank
        ...
        for i in range(num_updates_in_epoch):
            ...
            if self.global_step < self.policy_train_start_step:
                ...
                if self.clip_mode == "gipo":
                    ...
                    coeff = torch.exp(-0.5 * (torch.log(r_detach) / self.sigma) ** 2)
...
def main(args):
    ...
    trainer_group = [
        TrainerActor.remote(
            ...
            clip_mode=args.clip_mode, sigma=args.sigma
        )
    ...
    rollout_workers = [
        RolloutWorkerActor.remote(
            ...
            args.gamma, args.lambda_, args.reward_scale, torch_dtype, args.rollout_local_buf, task_ids
