...
from envs.diffusion.denoiser import Denoiser, SimpleBatch
...
def load_denoiser_from_checkpoint(
    ...
    if hasattr(agent_cfg.denoiser, '_target_'):
        ...
        if target.startswith('diffusion.') and not target.startswith('envs.diffusion.'):
            ...
            print(f"Fixed denoiser._target_: {target} -> {agent_cfg.denoiser._target_}")
    ...
    if hasattr(agent_cfg.denoiser.inner_model, '_target_'):
        ...
        if target.startswith('diffusion.') and not target.startswith('envs.diffusion.'):
            ...
            print(f"Fixed inner_model._target_: {target} -> {agent_cfg.denoiser.inner_model._target_}")
    ...
    if hasattr(trainer_cfg.world_model_env.diffusion_sampler, '_target_'):
        ...
        if target.startswith('diffusion.') and not target.startswith('envs.diffusion.'):
            ...
            print(f"Fixed diffusion_sampler._target_: {target} -> {trainer_cfg.world_model_env.diffusion_sampler._target_}")
    ...
    if hasattr(trainer_cfg.denoiser.sigma_distribution, '_target_'):
        ...
        if target.startswith('diffusion.') and not target.startswith('envs.diffusion.'):
            ...
            print(f"Fixed sigma_distribution._target_: {target} -> {trainer_cfg.denoiser.sigma_distribution._target_}")
...
def test_single_env_vs_batch():
    ...
    # 使用相同的 denoiser 实例进行比较（避免重复加载导致的问题）
    denoiser2 = denoiser1
...
def test_batch_functions():
    """测试新创建的 batch 函数"""
    import torch
    from envs.utils import tensor_to_image, tensor_to_image_batch
