# Code snapshot for experiment (Seed: 42)
# Saved at: 2026-01-07 17:02:32
# Original file: ds_metaworld_ppo_mlp_add_vtrace_with_param_more_stats.py
################################################################################
...
def main():
    ...
    # 如果外部已传入 SWANLAB_DIR（批量脚本会传入 task/swanlab_all），则优先使用
    ...
    if swanlab_dir_env:
        ...
        if TASK_NAME not in swanlab_dir_env and swanlab_dir_env.rstrip("/").endswith("swanlab_all"):
            ...
            debug_log(f"[修复] SwanLab 路径任务名不匹配，从 {swanlab_dir_env} 纠正为 {swanlab_base_dir}")
        else:
            # 使用批量脚本指定的共享目录
    ...
    train_group_port = find_free_port(base_port=train_base_port)
