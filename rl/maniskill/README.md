...
```bash
python rl/maniskill/smoke_maniskill_eval.py
```

它会构建 `PickCube-v1`，检查 `base_camera` 和 `hand_camera` 的图像 shape，并随机 step 几步。

使用 actor-critic checkpoint 做 rollout 评估：
...
如果要调试 PegInsertionSide 的 action chunk 执行方式，可以使用：

```bash
python rl/maniskill/maniskill_actor_critic_debug.py \
  --checkpoint /path/to/checkpoint \
  --output-dir /path/to/debug_output
```

GPU 渲染或 worker 绑定异常时，可以看：

```bash
python rl/maniskill/debug_rollout_render_gpu.py
```
...
这些内容已经适合放进项目根目录 `.gitignore`，代码仓库只保留脚本、配置和小型说明文件即可。

## 常见排查顺序

如果训练或评估失败，建议按下面顺序缩小问题：

1. 运行 `smoke_maniskill_eval.py`，确认 ManiSkill 环境和双相机观测可用。
2. 用 `check_replay_dataset.py` 检查 replay 后 H5 是否有正确相机、action 和 success 字段。
3. 用 `check_rlds_dataset.py` 检查 TFDS/RLDS 样本结构。
4. 用 `preprocess_rlds_to_pt.py --max_samples` 小样本预处理，确认 tokenizer/image pipeline 正常。
5. 小步数运行 `finetune_maniskill.py`，确认 SFT 能保存 checkpoint。
6. 小 worker 数运行 `ds_maniskill_ppo_discrete.py`，确认 rollout、inference 和 trainer 能连起来。
