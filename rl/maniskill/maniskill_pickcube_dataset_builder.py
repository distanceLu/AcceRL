"""TFDS builder for ManiSkill PickCube-v1 demonstrations (rgbd replay, dual camera).

Input : an h5 file produced by ``mani_skill.trajectory.replay_trajectory`` with
        ``--obs-mode rgbd`` using panda_wristcam robot (so every step has a
        224x224 RGB image from both base_camera and hand_camera, plus Panda
        state / tcp pose).
Output: an RLDS-style ``tfds`` dataset whose feature layout mirrors the LIBERO
        builder used in OpenVLA, so the existing loader (``prismatic.vla.datasets
        .rlds``) can pick it up with minor config changes.

Action space is ``pd_ee_delta_pose`` (7-D):
    [dx, dy, dz, drx, dry, drz, gripper]
with gripper in roughly [-1, +1] (+1 = open, -1 = close in ManiSkill).  The
values are stored *raw* (not clipped) so downstream normalization stats are
computed on the real policy distribution; any clipping to the env action space
is performed inside the dataset transform.

Per-step state layout (8-D) follows LIBERO:
    [tcp_x, tcp_y, tcp_z, tcp_rx, tcp_ry, tcp_rz, finger_left, finger_right]
where the first six are TCP xyz + axis-angle (converted from the ManiSkill wxyz
quaternion), and the last two are ``qpos[7:9]`` (both Panda fingers).
"""
...
# ---------------------------------------------------------------------------
DEFAULT_H5_PATH = (
    "/data/disk1/lcx_stu4/PickCube-v1/motionplanning/"
    "trajectory.rgbd.pd_ee_delta_pose.physx_cpu.h5"
)

IMG_SIZE = 224  # model input resolution; replay renders at this size directly
...
class ManiskillPickcube(tfds.core.GeneratorBasedBuilder):
    """ManiSkill PickCube-v1 imitation demos, RLDS-compatible (dual camera)."""

    VERSION = tfds.core.Version("2.0.0")
    RELEASE_NOTES = {
        "1.0.0": "Initial release of PickCube-v1 (Panda, pd_ee_delta_pose, rgbd replay).",
        "2.0.0": "Dual camera (base + wrist) at 224x224 with panda_wristcam.",
    }
            ...
            features=tfds.features.FeaturesDict({
                ...
                "episode_metadata": tfds.features.FeaturesDict({
                    ...
                    "success": tfds.features.Scalar(
                        dtype=np.bool_,
                        doc="Whether the final step is a successful PickCube demo.",
                    ),
                }),
            }),
    ...
    def _split_generators(self, dl_manager: tfds.download.DownloadManager):
        h5_path = os.environ.get("MANISKILL_PICKCUBE_H5", DEFAULT_H5_PATH)
