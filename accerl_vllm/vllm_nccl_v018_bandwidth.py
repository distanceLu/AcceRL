"""vLLM 0.18-compatible NCCL weight-transfer bandwidth test.

Run example:
CUDA_VISIBLE_DEVICES=1,2 python vllm_nccl_v018_bandwidth.py

This variant measures the vLLM 0.18 weight-transfer path:
init_weight_transfer_engine + update_weights + trainer_send_weights.
It intentionally does not call start_weight_update/finish_weight_update because
those worker methods are not implemented in vLLM 0.18.
"""
...
for transfer_idx in range(NUM_WEIGHT_TRANSFERS):
    print(
        ...
        f"(NCCL, packed=True, vLLM 0.18 API)"
    )
    ...
    core_total_seconds = update_seconds
